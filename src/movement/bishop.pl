...
%
%  Finds all valid moves for a bishop on the given Square.
%
%  @param State the current game state
%  @param Square the square that contains a bishop
%  @param Turn the owner of the bishop
%  @param Moves the resulting available moves
moves(State, Square, Turn, Moves) :-
  % Extract the board from the state.
  state:board(State, Board),
  % Get all bishop moves from the current square.
  positions:bishop_moves(Square, BishopMoves),
  % Validate every found bishop move.
